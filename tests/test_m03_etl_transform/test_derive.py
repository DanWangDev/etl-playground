...
def test_watch_hours_calculation(spark: SparkSession):
    ...
    rows = [
        Row(
            event_id="evt-001", content_id="C1", region="UK",
            completion_rate=0.75, watch_minutes=120.0,
            planned_launch_date="2026-01-01", actual_launch_date="2026-01-01",
        ),
    ]
...
def test_is_completed_flag(spark: SparkSession):
    ...
    rows = [
        Row(
            event_id="evt-001", content_id="C1", region="UK",
            completion_rate=1.0, watch_minutes=60.0,
            planned_launch_date=None, actual_launch_date=None,
        ),
        Row(
            event_id="evt-002", content_id="C2", region="US",
            completion_rate=0.5, watch_minutes=30.0,
            planned_launch_date=None, actual_launch_date=None,
        ),
    ]
    df = spark.createDataFrame(rows)
...
def test_launch_delay_null_when_missing_dates(spark: SparkSession):
    ...
    rows = [
        Row(
            event_id="evt-001", content_id="C1", region="UK",
            completion_rate=0.5, watch_minutes=60.0,
            planned_launch_date=None, actual_launch_date=None,
        ),
    ]
    df = spark.createDataFrame(rows)
